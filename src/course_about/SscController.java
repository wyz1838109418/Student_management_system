...
public class SscController {
    ...
    @FXML
    private void searchavg() throws ClassNotFoundException, SQLException {
        ObservableList<select_course> data = FXCollections.observableArrayList();
        String search_sno = fx_1.getText();
        // 1. 注册驱动
        Class.forName("com.mysql.cj.jdbc.Driver");// 建议写上，不写也不会报错
        ...
        Connection conn = DriverManager.getConnection(url, username, password);
        // 3. 定义sql
        String sql = "SELECT sno,Student_management.avgscore(sno) as score FROM Student_management.select_course WHERE sno = '" + search_sno + "'";
        Statement stmt = conn.createStatement();
        ResultSet rs = stmt.executeQuery(sql);
        if (rs.next()) {
            String sno = (String) rs.getString("sno");
            String score = (String) rs.getString("score");
            select_course s1 = new select_course(sno, "", score);
            data.add(s1);
        }
        fx_6.setItems(data);
    }
